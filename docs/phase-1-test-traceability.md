...
需求基线为 SRS V1.2。状态说明：“通过”表示当前自动化回归通过；“回归失败”表示已有测试稳定暴露实现或契约问题；“待浏览器验收”表示必须在可运行页面中补真实视口证据。
...
| FR-007 | 店铺分页、修改、资源不存在和所属权 | `ShopManagementServiceTest`、`ShopControllerContractTest`、`SrsV12ApiMappingContractTest`、`MerchantConsole.test.js` | 回归失败：公开店铺分页接口缺失；异常状态和防重复保存未处理 |
...
| FR-009 | 商品分页/详情、增删改查、可见性和资源归属 | `ProductServiceTest`、`ProductBoundaryServiceTest`、`ProductControllerContractTest`、`SrsV12ApiMappingContractTest`、`MerchantProducts.test.js` | 回归失败：分类分页、详情接口缺失；商家页异常状态未处理 |
| FR-010 | 商品价格范围、精度和独立修改接口 | `ProductPriceValidationTest`、`RequestDtoBoundaryTest`、`SrsV12ApiMappingContractTest` | 回归失败：价格校验通过，独立价格接口缺失 |
| FR-011 | 库存非负、商品与店铺校验 | `ProductStockServiceTest`、`ProductBoundaryServiceTest`、`ProductControllerContractTest` | 回归失败：库存操作返回资源不存在或抛错 |
| FR-012 | 添加购物车、用户/店铺隔离、数量合并和库存 | `CartAddServiceTest`、`CartBoundaryServiceTest`、`CartControllerContractTest`、`SrsV12ApiMappingContractTest`、`Shop.test.js` | 回归失败：写接口路径/方法不符；停业及临时闭店仍可加购 |
...
| FR-016 | 事务下单、初始状态、唯一订单号、金额和停业限制 | `OrderCreationServiceTest`、`OrderBoundaryServiceTest`、`OrderControllerContractTest` | 回归失败：初始状态仍为 `PENDING`；部分错误码未统一 |
| FR-017 | 订单列表、时间筛选、稳定排序及分页边界 | `OrderListServiceTest`、`OrderBoundaryServiceTest`、`SrsV12ApiMappingContractTest`、`Orders.test.js` | 回归失败：默认仍为 10，`size=101` 未拒绝；前端仍使用旧默认值/状态 |
...
| FR-020 | 统一响应、全失败追踪编号和内部信息保护 | `ApiResponseTest`、`SecurityIntegrationTest`、`SensitiveDataExposureTest`、`http.test.js` | 回归失败：缺通用 500 处理；诊断文本暴露密码/完整手机号；前端丢失 `traceId` |
