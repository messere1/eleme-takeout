...
| 401 | `AUTH_INVALID` | 未登录、凭证错误或 Token 无效 | 清理登录态并跳转登录页 |
...
| 商品 | GET | `/api/v1/shops/{shopId}/products?categoryId={id}` | 公开 | 查询分类下可见商品 |
...
| 购物车 | POST | `/api/v1/cart` | CUSTOMER | 添加商品 |
| 购物车 | GET | `/api/v1/cart` | CUSTOMER | 查询购物车 |
| 购物车 | PUT | `/api/v1/cart/items/{itemId}` | CUSTOMER | 修改数量；0 表示删除 |
...
`GET /api/v1/shops/{shopId}/products?categoryId=30`

公开接口，`categoryId` 必填，`data` 为 `ProductView[]`。列表只应返回可对顾客展示的商品。
...
- `PUT /api/v1/cart/items/{itemId}`，请求 `{"quantity":3}`；数量为 `0` 时删除该项。
...
```http
GET /api/v1/orders?status=PENDING&page=1&size=10&startTime=2026-09-01T00:00:00
```
