...
async function claim(o){try{await claimOrder(o.id);await load()}catch(e){message.value=e?.message||'接单失败'}}
async function deliver(o){try{await deliverOrder(o.id);await load()}catch(e){message.value=e?.message||'更新失败'}}
onMounted(load)
</script>
<template><section class="rider"><h2>骑手配送台</h2><p v-if="loading">加载中…</p><p v-if="message" class="error">{{message}}</p>
<h3>可接订单</h3><p v-if="!loading&&!available.length">暂无可接订单</p><div v-for="o in available" :key="o.id" class="card"><span>{{o.orderNo}}</span><button @click="claim(o)">接单</button></div>
<h3>我的配送</h3><p v-if="!loading&&!mine.length">暂无配送任务</p><div v-for="o in mine" :key="o.id" class="card"><span>{{o.orderNo}} · {{o.status}}</span><button v-if="o.status==='DELIVERING'" @click="deliver(o)">确认送达</button></div></section></template>
